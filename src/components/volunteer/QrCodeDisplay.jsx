...
function QrCodeDisplay({ participationId, eventData }) {
	...
	const { mutate: generateQR, isPending, error } = useVolunteerGenerateQrCodeMutation();
	...
	const handleDownload = () => {
		...
		link.href = pngUrl;
		link.download = `volunteer-qr-${qrCodeData?.user?.nama || "volunteer"}-${Date.now()}.png`;
		...
		return (
			<div className="bg-gradient-to-br from-emerald-50 to-teal-50 border-2 border-emerald-200 rounded-xl p-8 text-center shadow-sm">
				...
				<p className="text-gray-700 mb-6 max-w-md mx-auto">
					Klik tombol di bawah untuk menampilkan dan mengunduh QR Code kehadiran Anda
				</p>
				<DynamicButton
					variant="success"
					size="lg"
				...
				<div className="mt-6 bg-white border-2 border-emerald-200 rounded-xl p-4">
					...
					<ul className="text-xs text-emerald-800 space-y-2 text-left max-w-md mx-auto">
						<li className="flex items-start gap-2">
							...
							<span>Unduh dan simpan QR Code di HP Anda</span>
						</li>
						<li className="flex items-start gap-2">
							<span className="font-bold text-emerald-600 flex-shrink-0">2.</span>
							<span>Bawa saat kegiatan dimulai (screenshot atau print)</span>
						</li>
						<li className="flex items-start gap-2">
							<span className="font-bold text-emerald-600 flex-shrink-0">3.</span>
							<span>Tunjukkan ke petugas kegiatan untuk di-scan</span>
						</li>
						<li className="flex items-start gap-2">
		...
		return (
			<div className="bg-red-50 border-2 border-red-200 rounded-xl p-6 text-center shadow-sm">
				...
				<p className="text-red-600 text-sm mb-4">
					{localError || error?.message || "Terjadi kesalahan saat membuat QR Code"}
				</p>
	...
	if (!qrData) {
		return (
			<div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-6 text-center shadow-sm">
				...
				<p className="text-amber-800 font-semibold mb-2">QR Code Belum Tersedia</p>
				...
				<div className="text-center space-y-2">
					{qrCodeData?.user && (
						<>
							<div className="flex items-center justify-center gap-2 mb-3">
								...
								<p className="text-lg font-bold text-gray-900">{qrCodeData.user.nama}</p>
							</div>
					...
					{eventData?.judul && (
						<div className="mt-4 pt-4 border-t border-emerald-200">
							...
							<p className="text-sm font-semibold text-gray-800">{eventData.judul}</p>
						</div>
					)}
					{participationId && (
						<p className="text-xs text-gray-500 mt-3">Participation ID: #{participationId}</p>
					)}
				</div>
			</div>
			...
			<div className="space-y-4 px-2">
				...
				{qrCodeData?.info && (
					<div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
						...
						<p className="text-xs text-blue-800 leading-relaxed mb-3">{qrCodeData.info}</p>
					</div>
				)}
				...
				<div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-4">
					<p className="text-xs text-amber-800 leading-relaxed">
						<strong>⚠️ Catatan:</strong> QR Code ini adalah ID volunteer Anda yang unik dan
						terenkripsi. Jangan bagikan ke orang lain untuk mencegah penyalahgunaan.
					</p>
				</div>
			</div>
		</div>
